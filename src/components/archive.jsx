...
import { faBell,  faFileArrowUp, faTrashCan, faPalette, faThumbtack } from '@fortawesome/free-solid-svg-icons'
...
function ArchiveDiv({searchText,currentUserId}) {
  ...
  const handleChangeNotes = (id) =>{
    ...
    onValue(query,(snapShot)=>{
      ...
      Object.keys(data).map((project)=>{
        if(project === id){
          update(ref(db,`notes/${project}`),{archive : false})
        }
      })
    })
  }
  ...
  const handleChangeReminder = (id) =>{
    ...
    onValue(query,(snapShot)=>{
      ...
      Object.keys(data).map((project)=>{
        if(project === id){
          update(ref(db,`notes/${project}`),{archive : false,reminder : true})
        }
      })
    })
  }
  ...
  const handleChangeBin = (id) =>{
    ...
    onValue(query,(snapShot)=>{
      ...
      Object.keys(data).map((project)=>{
        if(project === id){
          update(ref(db,`notes/${project}`),{archive : false,bin : true})
        }
      })
    })
  }
  ...
  const handleChangePinTrue = (id) =>{
    ...
    onValue(query,(snapShot)=>{
      ...
      Object.keys(data).map((project)=>{
        if(project === id){
          return update(ref(db,`notes/${project}`),{pin : true, archive : false})
        }
      })
    })
  }
            ...
            notes.map((item)=>(
              <div className='note-item' key={item.id} style={{backgroundColor: item.backGroundColor}}>
                ...
                <div className='bin-option-icons'>
                  <span>
                    <FontAwesomeIcon icon={faFileArrowUp} onClick={()=>handleChangeNotes(item.id)}/>
                  </span>
                  <span>
